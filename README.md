...
이 레포는 제품 API 레포(`specpilot-ai`)와 분리된 웹 프론트입니다. 사용자는 구매 조건을 입력하고, 데스크톱 PC 또는 노트북 추천 결과, 구매 판정, 구매 타이밍, 공유 브리프, 상품 페이지 근거 검수, 결제 전 검수, 저장 리포트 기반 구매 상담, 피드백, 베타 신청, 요금제 관심 등록을 한 화면에서 처리합니다.
