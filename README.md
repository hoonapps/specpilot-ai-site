...
이 레포는 제품 API 레포(`specpilot-ai`)와 분리된 웹 프론트입니다. 첫 화면은 컴퓨터·노트북 구매자가 실제로 묻는 “이거 사도 되나?”에 답하는 질문-답변 데스크로 좁혔고, 세부 도구와 포트폴리오 설명은 별도 페이지로 나눴습니다.

## 제품 구조

- `/launch`: 구매 질문, 상품/장바구니 요약, 최종가, 리뷰 리스크를 입력하고 결제 가능 여부, 위험 신호, 추가 확인 증거, 복사용 답변을 받는 answer-first 데모
- `/launch/tools`: 구매 여정, 리뷰 리스크, 옵션/사양 검수, 최종 판정, 출시 운영 패널을 기능별로 확인하는 데모 룸
- `/launch/guide`: 무엇을 하려고 했고, 무엇을 만들었고, 어떻게 쓰며, 이력서에서 어떤 역량을 보여주는지 설명하는 포트폴리오 가이드
- `DESIGN.md`: design.md 방식처럼 YAML 토큰과 마크다운 근거를 함께 두고 색상, 타이포그래피, 레이아웃, 컴포넌트 기준을 관리하는 디자인 기준 문서
...
- `/launch`: `/api/specpilot/community-reply-kit`을 호출해 구매 질문, 상품/장바구니 요약, 최종가, 리뷰 리스크를 결제 가능 여부와 증거 요청이 포함된 답변으로 변환하는 공개 질문-답변 페이지
- `/launch/tools`: 기존 런칭룸의 기능별 공개 데모를 분리한 페이지. 구매 여정, 리뷰 리스크, 옵션/사양 검수, 최종 판정, 구매 실행, 출시 운영 패널을 필요할 때만 확인
- `/launch/guide`: 사용법, 제품 의도, 구현 범위, 포트폴리오 설명을 정리한 가이드
- `/launch/opengraph-image`, `/launch/twitter-image`: 커뮤니티와 메신저 미리보기에서 구매 질문 답변 제품임을 보여주는 1200x630 PNG 공유 이미지
...
`check:launch-visual`은 실행 중인 `/launch`를 Chrome headless로 열어 데스크톱/모바일 hero 제목, CTA, proof card, 질문-답변 데스크, 포트폴리오 섹션, 상단 라우팅 링크를 확인하고, 페이지 끝까지 스크롤해 가로 overflow, 텍스트 overflow, 폼 컨트롤 화면 이탈을 검사한 뒤 캡처를 저장합니다. `CHROME_PATH`와 `LAUNCH_VISUAL_OUT_DIR`로 Chrome 경로와 캡처 저장 위치를 지정할 수 있습니다.
