...
- `/api/specpilot/buyer-trust-kit`: 제품 API의 `/public/buyer-trust-kit`로 Trust Center를 첫 방문자용 신뢰 배지, 구매자 권리, 위험 고지, 쉬운 보증 문구로 압축 조회
...
- `/api/specpilot/ownership-cost-kit`: 제품 API의 `/public/ownership-cost-kit`로 구매가, 보유 기간, 유지비, 업그레이드비, 다운타임, 재판매율을 월 실질 비용과 감가 시나리오로 변환
- `/api/specpilot/warranty-return-kit`: 제품 API의 `/public/warranty-return-kit`로 반품/교환 기간, 초기 불량 예외, 개봉 후 반품, 보증 주체, 반품 비용, 위험 약관을 결제 전 보호 점수로 변환
- `/api/specpilot/price-breakdown-kit`: 제품 API의 `/public/price-breakdown-kit`로 표시가, 배송비, 조립비, OS 비용, 쿠폰, 카드 할인, 포인트, 수량을 최종 실구매가와 예산/리포트 가격 차이로 변환
- `/api/specpilot/deal-sanity-kit`: 제품 API의 `/public/deal-sanity-kit`로 컴퓨터/노트북 특가 후보의 기준가, 최근 최저가, 실구매가, 보증/반품, 판매자 평판, 위험 문구를 특가 안전성 점수와 결제 중단 규칙으로 변환
- `/api/specpilot/budget-stress-kit`: 제품 API의 `/public/budget-stress-kit`로 예산을 넘는 후보를 예산 유지, 증액, 조건 완화, 목표가 대기 시나리오와 결제 규칙으로 변환
...
- `/api/specpilot/spec-term-decoder-kit`: 제품 API의 `/public/spec-term-decoder-kit`로 FreeDOS, TGP, 온보드, 리퍼, 병행수입 같은 사양/구매 용어를 초보자용 설명, 위험도, 판매자 질문, 검수 prefill로 변환
- `/api/specpilot/purchase-question-triage-kit`: 제품 API의 `/public/purchase-question-triage-kit`로 자연어 구매 질문을 가격, 사양, 보증/반품, 결제 전 검수, 수령 후 점검 유형으로 분류하고 다음 키트와 복사용 질문을 생성
- `/api/specpilot/review-risk-kit`: 제품 API의 `/public/review-risk-kit`로 후기 문구, 평점, 리뷰 수에서 발열, 팬 소음, 초기 불량, 화면/패널, 배터리, AS 반복 불만을 구매 리스크 신호로 변환
...
- `/api/specpilot/checkout-lock-kit`: 제품 API의 `/public/checkout-lock-kit`로 1순위 후보의 잠금가, 사양, 판매자, 보증/반품 기준을 최종 결제 화면과 대조해 locked/verify/blocked, 중단 조건, 구매 실행 prefill로 변환
- `/api/specpilot/decision-defense-kit`: 제품 API의 `/public/decision-defense-kit`로 1순위 구매 결정을 가족/팀/커뮤니티에 공유하기 전 가격, 대체 후보, 용도, 리스크, 구매 타이밍 반박 질문과 방어 답변, 채널별 복사 문구로 변환
...
- `/api/specpilot/requirements-consensus-kit`: 제품 API의 `/public/requirements-consensus-kit`으로 가족/팀/커뮤니티 이해관계자의 예산, 용도, 필수 조건, 제외 조건을 합의 점수, 충돌, 분석 요청 prefill, 복사 문구로 변환
- `/api/specpilot/build-blueprint-kit`: 제품 API의 `/public/build-blueprint-kit`으로 예산과 용도를 부품/스펙 목표, 예산 배분, 가격비교 검색어, 피해야 할 조건, 세팅 호환성 prefill로 변환
...
- `/launch`: 제품 API의 `/public/launch-room`, `/public/start-concierge`, `/public/purchase-journey-kit`, `/growth/launch-experiment-dashboard`, `/growth/launch-experiments/{experiment_id}/events`, `/growth/launch-distribution-plan`, `/growth/retention-hub`, `/growth/referral-launch-kit`, `/growth/acquisition-hub`, `/growth/public-conversion-board`, `/growth/launch-pulse`, `/growth/launch-war-room`, `/growth/launch-incident-center`, `/growth/launch-week-recap`, `/growth/launch-community-kit`, `/growth/launch-media-kit`, `/growth/launch-activation-offer`, `/growth/launch-response-loop`, `/ops/public-launch-preflight`, `/ops/team-purchase-consult-kit`, `/beta/readiness`, `/beta/launch-gate`, `/public/buyer-checklist`, `/public/buyer-persona-quiz`, `/public/mistake-cost-calculator`, `/public/buyer-challenge-kit`, `/public/setup-compatibility-kit`, `/public/upgrade-readiness-kit`, `/public/ownership-cost-kit`, `/public/warranty-return-kit`, `/public/price-breakdown-kit`, `/public/deal-sanity-kit`, `/public/budget-stress-kit`, `/public/purchase-execution-kit`, `/public/final-decision-kit`, `/public/reviewer-quick-card-kit`, `/public/listing-decoder-kit`, `/public/spec-term-decoder-kit`, `/public/purchase-question-triage-kit`, `/public/review-risk-kit`, `/public/spec-risk-scanner`, `/public/candidate-compare`, `/public/deal-timing-window`, `/public/price-watch-kit`, `/public/social-proof-wall`, `/public/launch-objection-kit`, `/public/launch-share-pack`, `/public/launch-action-router`, `/public/launch-smoke`, `/public/proof-hub`, `/pricing/plans`를 읽어 공개 데모, 첫 구매 진단 콘시어지, 공개 구매 여정 오케스트레이터, 자연어 구매 질문 라우팅, 공개 리뷰 리스크 검수, CTA 실험 스트립, 첫 주 채널 배포 플랜, 구매 후속 리텐션 루프, 가입 전 추천 확산 키트, 공개 반응 운영 패널, 첫 24시간 런칭 워룸, 런칭 인시던트 센터, D+7 런칭 리포트, 커뮤니티 댓글 대응 키트, 런칭 미디어 키트, 런칭 전환 오퍼, 런칭 반응 후속 루프, 공개 출시 최종 체크, 런칭 반박 FAQ, 공유 확산팩, 방문자 액션 라우터, 공개 런칭 스모크 체크, Team 구매 표준안 프리뷰, 공개 출시 게이트, 구매 실패 방지 체크리스트, 30초 구매 성향 진단 퀴즈, 구매 실패 비용 계산기, 구매 챌린지 공유 키트, 공개 세팅 호환성 키트, 공개 업그레이드 수명 검수, 공개 총소유비용/재판매 가치 검수, 공개 보증/반품 정책 검수, 공개 실구매가 분해, 공개 특가 안전성 검수, 공개 예산/조건 스트레스 테스트, 공개 구매 실행 패키지, 공개 최종 구매 판정 키트, 공개 30초 검토 카드, 공개 상품명 해석 키트, 공개 사양 용어 해석 키트, 옵션/사양 빠른 검수기, 공개 후보 비교 스냅샷, 공개 구매 타이밍 윈도우, 공개 목표가 감시 키트, 공개 검증 허브 evidence kit, Free/Premium/Team 요금제 비교 카드, 실제 반응 proof, 첫 주 founder update, SEV/runbook/escalation, 반복 질문 답변 템플릿, 외부 소개 피치, 첫 CTA/전환 오퍼, proof 후보/founder reply/제품 수정 큐, 시장 리포트, 출시 CTA, 공유 문구를 한 화면에 보여주고, 상시 전환 바로 분석 시작/공유 문구/추천 대기열 CTA를 고정 노출하며 클릭을 성장 이벤트로 저장하고, 공유 확산팩은 절대 URL 복사와 네이티브 공유를 제공하며 모든 복사/공유/열기 액션을 `share_cta`로 저장하고, 첫 화면은 내부 운영 점수 대신 구매 데모, 시장 리포트, 검증 근거, 결제 전 체크 개수를 보여주며, 반응형 hero 배경과 CTA 줄바꿈 가드로 모바일 첫 화면의 잘림을 막고, 출시 게이트부터 요금제/배포 플랜까지 무거운 런칭 운영 패널 전체를 스크롤 기반 지연 마운트, 재검증 캐시, 타임아웃 fallback으로 분산해 첫 방문 체감을 유지하며, `/launch/opengraph-image`와 `/launch/twitter-image` 전용 공유 카드를 메타에 연결하고, hero와 데모 분석 CTA는 세션 handoff로 메인 분석 폼을 자동 채우면서 `analysis_view` 성장 이벤트를 남기며 방문자 액션 라우터 선택/CTA 클릭, 추천 대기열, 요금제 관심 등록을 제품 API에 바로 저장하는 외부 공유용 런칭룸
- `/launch`는 `/public/buyer-trust-kit`을 hero 직후에 렌더링해 가격/제휴/개인정보/사람 검수 기준과 구매자 권리를 분석 폼 진입 전 먼저 보여준다.
- `/launch`는 `/public/requirements-consensus-kit`을 구매 챌린지 다음에 렌더링해 가족/팀/커뮤니티 조건을 후보 검색 전에 합의하고 분석 handoff로 넘긴다.
- `/launch`는 `/public/build-blueprint-kit`을 조건 합의 다음에 렌더링해 가격비교 검색 전 부품/스펙 목표, 예산 배분, 제외 조건을 고정하고 세팅 호환성 검수로 이어준다.
...
- `/launch`는 `/public/ownership-cost-kit`을 업그레이드 수명 다음에 렌더링해 구매가, 유지비, 업그레이드비, 다운타임, 재판매 가치를 순비용/월 실질 비용/감가 시나리오로 변환한다.
...
- `/launch`는 `/public/deal-sanity-kit`을 실구매가 분해 다음에 렌더링해 컴퓨터/노트북 특가 후보의 가짜 할인, 조건부 할인, 리퍼/해외/반품 불가 위험을 구매 실행 전에 검수한다.
- `/launch`는 `/public/budget-stress-kit`을 특가 안전성 다음에 렌더링해 예산을 넘는 후보를 예산 유지, 증액, 조건 완화, 목표가 대기 시나리오로 비교하고 구매 실행 전 결제 규칙을 확정한다.
...
- `/launch`는 `/public/purchase-question-triage-kit`을 첫 구매 진단 다음에 렌더링해 방문자의 자연어 구매 질문을 가격, 사양, 보증/반품, 결제 전 검수, 수령 후 점검 유형으로 나누고 다음 공개 키트로 보낸다.
- `/launch`는 `/public/review-risk-kit`을 구매 질문 라우팅 다음에 렌더링해 사용자가 붙여 넣은 후기 문구에서 발열, 팬 소음, 초기 불량, 화면/패널, AS 반복 불만을 구매 리스크 신호로 바꾼다.
- `/launch`는 `/public/spec-term-decoder-kit`을 옵션/사양 빠른 검수 전에 렌더링해 FreeDOS, TGP, 온보드, 리퍼, 병행수입 같은 상품 문구를 쉬운 설명과 판매자 질문으로 바꾼다.
...
- `/launch`는 `/public/checkout-lock-kit`을 커스텀 후보 비교 다음에 렌더링해 고른 1순위가 최종 결제 화면에서 상품명, 판매자, 가격, 사양, AS/반품 조건이 바뀌지 않았는지 잠근다.
...
`check:launch-visual`은 실행 중인 `/launch`를 Chrome headless로 열어 데스크톱/모바일 hero 제목, CTA, proof pill, 상시 전환 바, 공유 확산팩 버튼/링크를 확인하고, 페이지 끝까지 스크롤해 공개 섹션 전체의 필수 섹션 누락, 가로 overflow, 텍스트 overflow, 폼 컨트롤 화면 이탈을 검사한 뒤 캡처를 저장합니다. `CHROME_PATH`와 `LAUNCH_VISUAL_OUT_DIR`로 Chrome 경로와 캡처 저장 위치를 지정할 수 있습니다.
...
- `POST /public/setup-compatibility-kit`
